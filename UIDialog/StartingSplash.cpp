...
BOOL StartingSplash::OnInitDialog() 
{
	...
	//----------------------------------------------------------------
	 pUserName="中华人民共和国广州仁爱医院 ";//北京-天坛
	 pAppName="量化靶控区域麻醉可视超声系统 ";
	 pDeveloperName="授权单位:曼翔(德国)医疗器械有限公司 ";
	 pCoprRightName="Version 1.0 Copyright@2012";
	 pUsableName="专为麻醉科设计的穿刺导航仪 使医生手指上长出“眼睛”";
	 //---------------------------------------------------------------
	 ...
	 NewOutTextFont.CreatePointFont(mUserNameFontCout,L"宋体");
	 ...
	 NewOutTextFont.CreatePointFont(mAppNameFontCout,L"宋体");
	 pOldFont=pDC->SelectObject(&NewOutTextFont);//选择你需要的字体 
	 mAppNameSize=pDC->GetTextExtent((CString)pAppName);//获得字符串长度

	 NewOutTextFont.DeleteObject(); 
	 NewOutTextFont.CreatePointFont(mFlagCharFontCout,L"宋体");
	 pOldFont=pDC->SelectObject(&NewOutTextFont);//选择你需要的字体 
	 mFlagCharSize=pDC->GetTextExtent((CString)pFlagChar);//获得字符串长度


	 NewOutTextFont.DeleteObject(); 
	 NewOutTextFont.CreatePointFont(mDeveloperNameFontCout,L"宋体");
	 pOldFont=pDC->SelectObject(&NewOutTextFont);//选择你需要的字体 
	 mDeveloperNameSize=pDC->GetTextExtent((CString)pDeveloperName);//获得字符串长度

	 NewOutTextFont.DeleteObject(); 
	 NewOutTextFont.CreatePointFont(mCoprRightNameFontCout,L"宋体");
	 pOldFont=pDC->SelectObject(&NewOutTextFont);//选择你需要的字体 
	 mCoprRightNameSize=pDC->GetTextExtent((CString)pCoprRightName);//获得字符串长度

	 NewOutTextFont.DeleteObject(); 
	 NewOutTextFont.CreatePointFont(mUsableNameFontCout,L"宋体");
	 pOldFont=pDC->SelectObject(&NewOutTextFont);//选择你需要的字体 
